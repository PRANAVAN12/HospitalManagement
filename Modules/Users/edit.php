...
{	
	...
	if(empty($username) || empty($email) || empty($mobile) || empty($password)|| empty($fullname || empty($gender) || empty($age))) {
		...
		//updating the table
		$result = mysqli_query($mysqli, "UPDATE users SET fullname='$fullname',username='$username',email='$email',mobile='$mobile' ,age='$age',gender='$gender',password='$password' WHERE id=$id");
